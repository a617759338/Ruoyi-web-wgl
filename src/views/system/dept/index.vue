...
const data = reactive({
  ...
  rules: {
    ...
    phone: [{ pattern: /^1[3|4|5|6|7|8|9]\d{8}$/, message: "请输入正确的手机号码", trigger: "blur" }]
  },
})
...
const handleUserSelected = (res) => {
  ...
  // ✅ 直接赋值文本，实时显示（关键修复）
  form.value[target + 'Text'] = res.nickNameStr
}
...
async function handleUpdate(row) {
  ...
  getDept(row.deptId).then(res => {
    form.value = { ...form.value, ...res.data } // 安全赋值
    const data = res.data

    // 统一回显名字
    form.value.assistantText = parseWorkCodeToName(data.assistant, userList.value)
    form.value.managerText = parseWorkCodeToName(data.manager, userList.value)
    form.value.directorText = parseWorkCodeToName(data.director, userList.value)
    form.value.leaderText = parseWorkCodeToName(data.leader, userList.value)
...
function parseWorkCodeToName(workCodeStr, userList) {
  ...
  return userList
      .filter(u => codes.includes(u.workCode + "") || codes.includes(u.userId + ""))
